      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="bg-white rounded-3xl p-8 md:p-12 shadow-sm border border-gray-100 flex flex-col lg:flex-row gap-12">
          ...
          <div className="w-full lg:w-1/2 flex flex-col gap-4">
            ...
            {/* Thumbnails (Dummy repetition for visual) */}
            <div className="flex gap-4 overflow-x-auto pb-2">
              {[1, 2, 3, 4].map((thumb) => (
                <button key={thumb} className={`w-24 h-24 rounded-xl border-2 flex-shrink-0 bg-gray-50 p-2 ${thumb === 1 ? 'border-blue-500' : 'border-transparent hover:border-gray-200'}`}>
                   <img 
                    src={product.image} 
                    alt={`Thumbnail ${thumb}`} 
                    className="w-full h-full object-contain"
                    onError={(e) => { (e.target as HTMLImageElement).src = 'https://placehold.co/100x100?text=Thumb'; }}
                  />
                </button>
              ))}
            </div>
